...
# dataset = dataset.mask(dataset['TFLYER']<0.1).mask(dataset['VELF']<0.03).dropna()
# dataset = dataset.mask(dataset['TEPLAETA']<1e-3).mask(dataset['TEPLAPHI0']>1e-4).mask(dataset['VELF']<0.02).dropna()
# dataset = dataset.mask(dataset['TEPLAETA']<1e-3).mask(dataset['VELF']<0.015).dropna()
# dataset = dataset.mask(dataset['VELF']>0.04).dropna()
# dataset = dataset.mask(dataset['TFLYER']>0.154).mask(dataset['min_poros']==dataset['max_poros']).dropna()
...
# dataset = dataset.iloc[:,vel_start_index+1:] ## drop porosity data, keep surface velocity
# dataset = dataset.iloc[:,[5,6,7,11,12,13,14,15]]
# dataset = dataset.iloc[:,[0,1,2,3,4,6,7,8,9,11,12,13,14,15]]

## uncomment to drop velocity data
# dataset.drop(dataset.columns[vel_start_index:-5],axis=1,inplace=True) ## drop surface velocity data, keep porosity
# dataset.drop(dataset.columns[50:vel_start_index],axis=1,inplace=True)
...
testdata.to_csv('testdata_tepla.csv',index=False)
# remember training data
trainingdata = pd.concat([pd.DataFrame(x_train),pd.DataFrame(y_train)],axis=1)
trainingdata.columns = dataset.columns
trainingdata.to_hdf('trainingdata_tepla.hdf5','trainingdata',index=False)
#########
...
fig, ax = plt.subplots(figsize=(7.5, 5), sharex=True)
...
# plt.grid()
plt.savefig('learning_history_3p.pdf',bbox_inches='tight')
plt.show()
...
model.save("model_tepla.h5")



fig, (ax1, ax2, ax3) = plt.subplots(1, 3, constrained_layout=False, figsize=(16,4))
errors = (np.abs(np.subtract(y_oI, y_pI)/ y_oI))

ax1.set_title(r'$Y_\mathrm{S}$',fontsize=fntsize)
ax1.plot(y_oI[:,2],y_pI[:,2],'ro',alpha = 0.5,label=f'{np.mean(errors[:,-1])*100:.2f} % mean error')
ax1.plot(y_oI[:,2],y_oI[:,2],'r-')
# ax1.axis((0,np.max(y_oI[:,2]),0,np.max(y_oI[:,2])))
...
ax2.plot(y_oI[:,1],y_pI[:,1],'bo',alpha = 0.5,label=f'{np.mean(errors[:,-2])*100:.2f} % mean error')
ax2.plot(y_oI[:,1],y_oI[:,1],'b-')
...
ax3.plot(y_oI[:,0],y_pI[:,0],'ko',alpha = 0.5,label=f'{np.mean(errors[:,-3])*100:.2f} % mean error')
ax3.plot(y_oI[:,0],y_oI[:,0],'k-')
...
fig.text(0.5, -0.03, 'Actual value', ha='center',fontsize=fntsize)
fig.text(0.08, 0.5, 'Deep NN prediction', va='center', rotation='vertical',fontsize=fntsize)
plt.savefig('teplaparam_prediction_deepnn.pdf', bbox_inches='tight')
plt.close()


# write predicted parameters of test data to csv file
predicted_data = pd.DataFrame(y_pI)
predicted_data.columns = dataset.columns[-3:]
predicted_data.to_csv('predicted_tepladata.csv',index=False)
